      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
            {filtered.map(item => (
              <div key={item.id}
                ...
                style={{
                  background: selected?.id === item.id ? '#1e3a5f' : '#0f172a',
                  border: `1px solid ${selected?.id === item.id ? '#2563eb' : '#1e293b'}`,
        ...
        {selected && (
          <div style={{
            ...
            <div style={{ fontSize: '12px', color: '#e2e8f0', lineHeight: 1.6, marginBottom: '4px' }}>
              {selected.summary}
            </div>
            {selected.summary_en && (
              <div style={{ fontSize: '11px', color: '#94a3b8', lineHeight: 1.5, marginBottom: '10px', fontStyle: 'italic' }}>
                {selected.summary_en}
              </div>
            )}
            {detailBody ? (
